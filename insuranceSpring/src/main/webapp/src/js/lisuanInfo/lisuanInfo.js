	function caronloads(indexPage){
			$("#carinfo").empty();
			$.ajax({
				url:'http://localhost:8080/loadLisuanInfo.do',
				type:'post',
				data:{
					'indexPage':indexPage
				},
				...
				success : function(data) {
					...
					for (var i = 0; i < data.datas.length; i++) {     
						...
						rowHtml+="<td > <img width='200' height='300' alt='' src='../../../"+data.datas[i].specificList+"'/></td>";
						rowHtml+="<td > <img width='200' height='300' alt='' src='../../../"+data.datas[i].specificInvoice+"'/></td>";
						
						rowHtml+="<td >"+data.datas[i].remark+"</td>";
						rowHtml+="<td><a href='#' id='' onclick='deleteLisuanInfo("+data.datas[i].dataId+");'>删除</a>";
						rowHtml+="<a href='http://localhost:8080/toupdateLisuanInfo.do?id="+data.datas[i].dataId+"' id=''>修改</a></td></tr>";
						contentHtml += rowHtml;			
					}
		...
		function deleteLisuanInfo(id){
			if(window.confirm("你确定要删除吗？")){
				$.ajax({
					url:'http://localhost:8080/deleteLisuanInfo.do',
   					type:'post',
					data:{'id':id},
					dataType : 'json',
					success : function(data) {
